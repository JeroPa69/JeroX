...
def signal(Px, window):
    z_score = zsc(Px, window)
    return -rank_w(z_score)
...
def objective(x):
    ...
    train = pd.concat([strat_ret['1995':'2000'], strat_ret['2005':'2010'],strat_ret['2015':'2020']], axis=0)
    tratio = np.nanmean(train)/np.nanstd(train)* np.sqrt(250) * (len(train.dropna())/250)**0.5
    return -tratio
