...
[weights1,weights2,weights3,weights4,weights5,weights6,weights7,weights8,
 weights10,weights11,weights16,weights18,weights19,weights20,weights21,weights22,
 weights30,weights26,weights36,weights35,
 weights34,weights38,weights40,weights41,
 weights42,weights43,weights44,weights45,weights46,
 weights47,weights48,weights49,weights51]

alphas = [weights1, weights5, weights30, weights38, weights52, weights53]
...
weights = getweights(vol_adj_alphas.drop(['RON', 'INR'], axis=1))
positions = round(exposure * weights * 5/VOL)
sim_tr = simulate_px(TR, S3M, n_vols=1, freq=252, sample_size=252)[0]

fx_pnl = (positions * np.log(sim_tr.astype(float)).diff().drop(['RON', 'INR'], axis=1)) - \
         (abs(positions.diff()) * slippage.drop(['RON', 'INR'], axis=1))

t_costs = abs((positions.diff()) * slippage.drop(['RON', 'INR'], axis=1)).sum(axis=1)

pnl = ((positions * np.log(sim_tr.astype(float)).diff().drop(['RON', 'INR'], axis=1)).sum(axis=1) - t_costs)
...
fx_pnl = ((positions * np.log(sim_tr.astype(float)).diff().drop(['RUB', 'INR', 'RON'], axis=1)) - \
...
pnl = ((positions * np.log(sim_tr.astype(float)).diff().drop(['RUB', 'INR', 'RON'], axis=1)).sum(axis=1) - t_costs)
